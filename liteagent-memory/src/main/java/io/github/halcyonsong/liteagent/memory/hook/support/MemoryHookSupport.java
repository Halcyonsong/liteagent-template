...
/**
 * 记忆窗口 Hook 共享逻辑。Chat 和 Stream 两侧 Hook 共享读取/写回/折叠逻辑，仅 context 类型不同。
 */
@Slf4j
public final class MemoryHookSupport {
    ...
    public static void loadHistory(MemoryWindowStore store, Invocation invocation, List<Message> workingMessages) {
        ...
        log.debug("Memory loaded. session={}, history={}, working={}",
                sessionId,
                history.size(),
    ...
    public static void saveRound(MemoryWindowStore store, Invocation invocation,
        ...
        log.debug("Memory saved. session={}, folded={}, window={}",
                sessionId,
                folded.size(),
                window.size()
        );
    ...
    /**
     * 折叠消息列表：system/tool 跳过，连续 assistant 合并为一条（换行分隔），user 保留。
     * 历史消息已折叠过，线性扫描不会重复合并。
     */
