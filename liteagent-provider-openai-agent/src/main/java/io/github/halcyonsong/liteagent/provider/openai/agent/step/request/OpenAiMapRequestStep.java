 ...
 * <p>
 * 该步骤会将中间产物写入 AgentContext.attributes，供后续步骤复用。
 */
public class OpenAiMapRequestStep implements AgentStep {

    private final OpenAiChatRequestMapper requestMapper;

    public OpenAiMapRequestStep(OpenAiChatRequestMapper requestMapper) {
    ...
    public AgentStepKey invoke(AgentContext context) {
        ...
        context.setAttribute(OpenAiAgentAttributes.PROVIDER_REQUEST, providerRequest);
        context.setAttribute(OpenAiAgentAttributes.RAW_REQUEST, rawRequest);

        return AgentStepKey.ENHANCE_REQUEST;
    }
}