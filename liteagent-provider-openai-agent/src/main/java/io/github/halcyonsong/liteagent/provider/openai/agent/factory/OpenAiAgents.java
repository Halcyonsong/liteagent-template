 ...
 */
public final class OpenAiAgents {

    private OpenAiAgents() {
    }

    /**
     * 基于现成的 WebClient 创建 OpenAiAgent。
     * <p>
     * 适合调用方已经自行管理 WebClient 生命周期的场景。
     *
     * @param webClient 已配置完成的 WebClient
     * @return 完成最小装配的 OpenAiAgent
     */
    public static OpenAiAgent create(WebClient webClient) {
        return create(webClient, List.of(), 1000);
    }

    /**
     * 基于现成的 WebClient 创建 OpenAiAgent，并允许传入最大步骤数。
     *
     * @param webClient 已配置完成的 WebClient
     * @param maxStepCount 单次执行允许的最大步骤数
     * @return 完成装配的 OpenAiAgent
     */
    public static OpenAiAgent create(WebClient webClient,int maxStepCount) {
        return create(webClient, List.of(), maxStepCount);
    }

    /**
     * 基于现成的 WebClient 创建 OpenAiAgent，并允许传入 hook 与最大步骤数。
     *
     * @param webClient 已配置完成的 WebClient
    ...
    public static OpenAiAgent create(WebClient webClient,
        ...
        OpenAiAgentExecutorFactory executorFactory = new OpenAiAgentExecutorFactory(
                requestMapper,
                clientSupport,
                chatTransport,
                responseMapper
        ...
        AgentExecutor executor = executorFactory.create(hooks, maxStepCount);
        Agent agent = new Agent(executor);
        return new OpenAiAgent(agent);
    }

    /**
     * 基于运行时配置创建 OpenAiAgent。
     * <p>
     ...
     * @return 完成最小装配的 OpenAiAgent
     */
    public static OpenAiAgent create(HttpRuntimeConfig runtimeConfig) {
        return create(runtimeConfig, List.of(), 1000);
    }

    public static OpenAiAgent create(HttpRuntimeConfig runtimeConfig,int maxStepCount) {
        return create(runtimeConfig, List.of(), maxStepCount);
    }

    /**
     * 基于运行时配置创建 OpenAiAgent，并允许传入 hook 与最大步骤数。
     *
     * @param runtimeConfig HTTP 运行时配置
     * @param hooks 步骤生命周期钩子
     * @param maxStepCount 单次执行允许的最大步骤数
