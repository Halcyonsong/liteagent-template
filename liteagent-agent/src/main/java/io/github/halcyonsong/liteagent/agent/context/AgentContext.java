package io.github.halcyonsong.liteagent.agent.context;
 ...
 * 单次 agent 调用的上下文。
 * <p>
 * 该对象的生命周期限定在一次 execute / streamExecute 调用内部，
 * 不是全局共享状态，也不会跨请求复用。
 * <p>
 * 它主要承载三类数据：
 * <ul>
 *     <li>本次调用输入：{@code invocation}</li>
 *     <li>执行控制状态：轮次、最大轮次、终止原因</li>
 *     <li>跨步骤共享的扩展数据：{@code attributes}</li>
 * </ul>
 * <p>
 * provider-specific 的中间产物不直接定义为字段，
 * 而是优先通过 {@code attributes} 扩展槽存储。
 */
@Getter
public class AgentContext {
    ...
    private AgentContext(String executionId, Invocation invocation) {
        this.executionId = executionId;
        this.invocation = invocation;
    }

    public static AgentContext create(Invocation invocation) {
        return create(UUID.randomUUID().toString(), invocation);
    }

    public static AgentContext create(String executionId, Invocation invocation) {
        ...
        return new AgentContext(executionId, invocation);
    }
